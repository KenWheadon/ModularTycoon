...
class BurgerFlippingModule extends GameModule {
  constructor() {
    super(GAME_CONFIG.MODULES.BURGER, "Burger Flipper", "🍔", {
                    ...
                    <div class="inventory-item">
                        ...
                        <button onclick="window.burgerModule.buyIngredient('tomatoes', 5)" 
                                ${
                                  ...
                                  this.ingredientCosts.tomatoes * 5
                                    ? ""
                                    : "disabled"
                                }>
            ...
            <div class="burger-kitchen">
                ...
                <div class="grill-station">
                    ...
                    <button class="add-to-grill-btn" onclick="window.burgerModule.addToGrill()" 
                            ${
                              this.getShapedPatties().length > 0 &&
                              this.activePatties.length < 4
                                ? ""
                                : "disabled"
                            }>
                        Add Patty to Grill
  ...
  renderRawMeatQueue() {
    ...
    return this.rawMeatQueue
      .map((meat) => {
        if (meat.isShaping) {
          return `
                    <div class="shaping-meat active" onclick="window.burgerModule.clickShape(${
                      meat.id
                    })">
                        <div class="meat-progress">
                            <div class="progress-bar" style="width: ${
                              (meat.shapeProgress / meat.targetShape) * 100
                            }%"></div>
                        </div>
                        <div class="shape-instruction">Click to shape! (${
                          meat.shapeProgress
                        }/${meat.targetShape})</div>
                    </div>
                `;
        } else {
          return `
    ...
    return this.activePatties
      .map((patty) => {
        ...
        const cookProgress = Math.min(
          100,
          (timeElapsed / this.pattyPerfectTime) * 100
        );
        ...
        if (patty.status === "burned") {
          ...
          status = `🍳 Cooking... (${Math.ceil(
            (this.pattyCookTimeMin - timeElapsed) / 1000
          )}s)`;
          className += "cooking";
        }
    ...
    if (this.inventory.meat > 0 && this.rawMeatQueue.length < 3) {
      ...
      setTimeout(() => {
        ...
        if (meat && meat.isShaping) {
          this.rawMeatQueue = this.rawMeatQueue.filter(
            (m) => m.id !== meatData.id
          );
  ...
  addToGrill() {
    ...
    if (shapedPatties.length > 0 && this.activePatties.length < 4) {
      ...
      setTimeout(() => {
        const activePatty = this.activePatties.find(
          (p) => p.id === grillPatty.id
        );
  ...
  flipPatty(pattyId) {
    ...
    if (patty && patty.status === "cooking") {
      ...
      if (
        timeElapsed >= this.pattyCookTimeMin &&
        timeElapsed <= this.pattyCookTimeMax
      ) {
        patty.status = "ready";
  ...
  assembleBurger() {
    if (this.canMakeBurger()) {
      ...
      this.activePatties = this.activePatties.filter(
        (p) => p.id !== readyPatty.id
      );
      this.inventory.buns--;
      this.inventory.tomatoes--;
      this.inventory.cheese--;
    ...
    setInterval(() => {
      if (this.finishedBurgers > 0) {
        ...
        window.showNotification(
          `💰 Sold ${burgersToSell} burger(s) for $${earnings}!`
        );
        window.game.render();
      }
