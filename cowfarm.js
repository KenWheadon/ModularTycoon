...
class CowFarmModule extends GameModule {
        ...
        return `
            ...
            <div class="module-stats">
                <div class="stat">
                    ...
                    <div class="stat-value">${this.cows.length}/${this.maxCows}</div>
                </div>
                <div class="stat">
                ...
                <div class="farm-actions">
                    <div class="farm-action">
                        ...
                        <button onclick="window.cowModule.buyCow()" 
                                ${moduleManager.currencyManager.getCurrency('dollars') >= this.cowCost && this.cows.length < this.maxCows ? '' : 'disabled'}>
                            ${this.cows.length >= this.maxCows ? 'Max Cows' : 'Buy Cow'}
                        </button>
                    </div>
                    <div class="farm-action">
                        <span>Buy 10 Hay Bales - $${this.hayBalesCost}</span>
                        <button onclick="window.cowModule.buyHay()" 
                                ${moduleManager.currencyManager.getCurrency('dollars') >= this.hayBalesCost ? '' : 'disabled'}>
                            Buy Hay
                        </button>
                    </div>
                    <div class="farm-action">
            ...
            switch(cow.status) {
                case 'idle':
                    ...
                    cowAction = `
                        ...
                        <button class="feed-cow-btn" onclick="window.cowModule.feedCow(${cow.id})" 
                ...
                case 'eating':
                    const eatTimeLeft = Math.max(0, cow.eatStartTime + this.cowEatingTime - timeNow);
                    const eatProgress = Math.min(100, ((this.cowEatingTime - eatTimeLeft) / this.cowEatingTime) * 100);
                    cowDisplay = '🐄';
                    cowClass += ' eating-animation';
                    cowAction = `
                        <div class="cow-status">🌾 Eating... (${Math.ceil(eatTimeLeft/1000)}s)</div>
                ...
                case 'gassy':
                    ...
                    cowAction = `
                        <div class="cow-status">💨 Gassy! Click to release! (${cow.gasCount}/${this.maxGasCount})</div>
                        <button class="gas-release-btn" onclick="window.cowModule.releaseGas(${cow.id})">
                            💨 Release Gas!
                        </button>
                        <div class="gas-counter">
                            ${'🟢'.repeat(cow.gasCount)}${'⚪'.repeat(this.maxGasCount - cow.gasCount)}
                        </div>
                    `;
                    break;
                ...
                case 'being_milked':
                    const milkProgress = Math.max(0, (this.maxMilkLevel - cow.milkLevel) / this.maxMilkLevel * 100);
                    cowDisplay = '🐄💧';
                    ...
                    cowAction = `
                        ...
                        <div class="milk-level">Milk: ${this.maxMilkLevel - cow.milkLevel}/${this.maxMilkLevel}</div>
                    `;
                    break;
                    
                case 'cooldown':
                    const cooldownLeft = Math.max(0, cow.cooldownEndTime - timeNow);
                    const cooldownProgress = Math.min(100, ((this.cowCooldownTime - cooldownLeft) / this.cowCooldownTime) * 100);
                    cowDisplay = '🐄😴';
                    cowAction = `
                        <div class="cow-status">😴 Resting (${Math.ceil(cooldownLeft/1000)}s)</div>
    ...
    // Buy a cow
    buyCow() {
        if (window.game.currencyManager.spend({dollars: this.cowCost}) && this.cows.length < this.maxCows) {
            const newCow = {
                id: this.nextCowId++,
                status: 'idle',
                gasCount: 0,
    ...
    startMilkSelling() {
        setInterval(() => {
            if (this.milkBottles > 0) {
                const milkToSell = Math.min(this.milkBottles, 5 + Math.floor(Math.random() * 10)); // Sell 5-14 bottles
                ...
                window.showNotification(`🥛 Sold ${milkToSell} milk bottles for $${earnings}!`);
                window.game.render();
            }
    ...
    updateCowStates() {
        ...
        this.cows.forEach(cow => {
            ...
            if (cow.status === 'eating' && now >= cow.eatStartTime + this.cowEatingTime) {
                cow.status = 'gassy';
                cow.gasCount = 0;
                needsUpdate = true;
            }
