...
class NFTCardsModule extends GameModule {
  constructor() {
    super(GAME_CONFIG.MODULES.NFT, "NFT Cards", "🃏", {
            ...
            <div class="nft-shop">
                ...
                <div class="shop-actions">
                    <div class="shop-action">
                        <span>Buy Card Pack (${this.cardsPerPack} cards) - ${
      this.packCost
    } DC</span>
            ...
            <div class="card-collection">
                ...
                <div class="collection-filter">
                    <button onclick="window.nftModule.filterCollection('all')" class="filter-btn active">All (${
                      this.cardCollection.length
                    })</button>
            ...
            <div class="nft-marketplace">
                ...
                <div class="marketplace-info">
                    <p>Listed: ${this.marketplace.length}/${
      this.maxMarketplaceSlots
    } slots</p>
                </div>
  ...
  generateRandomCard() {
    ...
    const randomName =
      rarityData.names[Math.floor(Math.random() * rarityData.names.length)];
    const baseValue = Math.floor(
      (Math.random() * 20 + 10) * rarityData.valueMultiplier
    );
    ...
    return {
      ...
      marketValue:
        baseValue +
        Math.floor(Math.random() * baseValue * 0.5 - baseValue * 0.25), // ±25% variance
    };
  }
  ...
  showPackOpening(cards) {
    ...
    cards.forEach((card, index) => {
      setTimeout(() => {
        const message = `${card.emoji} ${rarityNames[card.rarity]}: ${
          card.name
        }!`;
    ...
    if (legendaryCards.length > 0) {
      setTimeout(() => {
        window.showNotification(
          `🎉 LEGENDARY PULL! You got ${legendaryCards.length} legendary card(s)!`
        );
  ...
  renderCardCollection() {
    ...
    const availableCards = this.cardCollection.filter(
      (card) => !this.marketplace.some((listing) => listing.cardId === card.id)
    );
    ...
    return availableCards
      .map((card) => {
        return `
                <div class="nft-card ${card.rarity}" data-rarity="${
          card.rarity
        }">
                    ...
                    <div class="card-actions">
                        <button class="list-card-btn" onclick="window.nftModule.listCard(${
                          card.id
                        })"
                                ${
                                  this.marketplace.length <
                                  this.maxMarketplaceSlots
                                    ? ""
                                    : "disabled"
                                }>
  ...
  renderMarketplace() {
    ...
    return this.marketplace
      .map((listing, index) => {
        ...
        const profitColor =
          listing.offer > listing.listingPrice
            ? "#00ff9f"
            : listing.offer < listing.listingPrice
            ? "#ff6b6b"
            : "#ffd93d";
  ...
  generateOffers() {
    this.marketplace.forEach((listing) => {
      if (!listing.hasOffer && Math.random() > 0.4) {
        // 60% chance
        const card = this.cardCollection.find((c) => c.id === listing.cardId);
        if (card) {
  ...
  acceptOffer(listingIndex) {
    ...
    if (listing && listing.hasOffer) {
      ...
      window.showNotification(
        `💰 Sold "${card.name}" for $${listing.offer}! ${profitText}`
      );
      window.game.render();
    }
  }
